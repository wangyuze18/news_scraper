...
### 更新说明
- **图片下载功能**：新增了 `download_images` 和 `image_save_dir` 参数，支持将图片保存到本地，并在输出中记录本地路径。
- **文件结构**：明确了图片保存目录的默认和自定义路径。
- **示例更新**：JSON 和 CSV 示例中增加了 `local_images` 和 `本地图片路径` 字段。
- **注意事项**：补充了图片下载相关的注意事项，如文件名格式和错误处理。
